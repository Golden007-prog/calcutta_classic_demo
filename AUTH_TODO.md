...
| Proxy (middleware) | — removed for the static GitHub Pages export (middleware needs a server); recreate `proxy.ts` per step 4 when auth goes live on Vercel |
...
4. **Session handling**: deploy to a server runtime (Vercel), recreate
   `proxy.ts` (`export default function proxy(request)` gating
   `/auth/:path*`), add `@supabase/ssr` with a server client, and refresh
   sessions there. Static GitHub Pages hosting can't run middleware.
